  ...
  try {
    ...
    const agarHtml = `
      ...
      <body style="margin:0;padding:0;background:#F5F0E8;font-family:'Georgia',serif;">
        <div style="max-width:560px;margin:40px auto;background:#fff;">
          <div style="background:#2f210c;padding:40px;text-align:center;">
            <h1 style="color:#fff;font-size:24px;font-weight:400;margin:0;letter-spacing:0.15em;text-transform:uppercase;">Ny bokning!</h1>
          </div>
          <div style="padding:40px;">
            <table style="width:100%;border-collapse:collapse;">
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
                <td style="padding:10px 0;color:#8B6F5E;font-size:13px;">Kund</td>
                <td style="padding:10px 0;font-size:13px;color:#2f210c;text-align:right;">${namn} ${efternamn}</td>
              </tr>
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
                <td style="padding:10px 0;color:#8B6F5E;font-size:13px;">Email</td>
                <td style="padding:10px 0;font-size:13px;color:#2f210c;text-align:right;">${email}</td>
              </tr>
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
                <td style="padding:10px 0;color:#8B6F5E;font-size:13px;">Telefon</td>
                <td style="padding:10px 0;font-size:13px;color:#2f210c;text-align:right;">${telefon}</td>
              </tr>
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
                <td style="padding:10px 0;color:#8B6F5E;font-size:13px;">Tjänst</td>
                <td style="padding:10px 0;font-size:13px;color:#2f210c;text-align:right;">${tjanst.namn}</td>
              </tr>
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
                <td style="padding:10px 0;color:#8B6F5E;font-size:13px;">Datum</td>
                <td style="padding:10px 0;font-size:13px;color:#2f210c;text-align:right;">${datumFormaterat}</td>
              </tr>
              <tr style="border-bottom:1px solid rgba(139,111,94,0.1);">
              ...
              <tr>
                ...
                <td style="padding:10px 0;font-size:16px;color:#B8956A;text-align:right;">${tjanst.pris}</td>
              </tr>
            </table>
            ${meddelande ? `<p style="margin-top:24px;padding:16px;background:#F7F3EC;font-size:13px;color:#8B6F5E;"><strong>Meddelande:</strong> ${meddelande}</p>` : ''}
          </div>
        </div>
      </body>
      </html>
    ...
    await resend.emails.send({
      ...
      subject: `Ny bokning — ${namn} ${efternamn} · ${tjanst.namn} ${datumFormaterat}`,
      html: agarHtml,
    })
