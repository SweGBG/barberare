...
## Designsystem (Svart & Guld — renovering juli 2026)
- Färger: bg #0A0908, panel #14110C, gold #C9A24B, gold-dim #E3C57B, text #EDE3CB, muted #9A8E77
- Guldgradient: var(--gold-grad) = linear-gradient(160deg, #E9CD8C, #C9A24B, #96712F)
- OBS: var(--dark) är numera LJUS textfärg (#EDE3CB), var(--cream)/var(--bg) är mörka bakgrunder — inverterade tokens så gamla moduler funkar
- Typsnitt: Playfair Display (rubriker, guld, uppercase), Raleway (UI)
- Ornamentlogotypen: /public/atilli-logo.png (hero, mix-blend-mode lighten + radial mask)
- Startsidan: Hero (logotyp + text) → Features → Tjänster (Supabase services, fallback vid fel) → Galleri → Om → Prislista → Boka-CTA → Footer (Supabase opening_hours, fallback vid fel)
- Admin använder CSS modules (*.module.css) — EJ omgjord i denna renovering
- Publik sida: CSS modules, mobilmeny slide-down, breakpoints 1100/768/480px, min 44-48px touch targets
